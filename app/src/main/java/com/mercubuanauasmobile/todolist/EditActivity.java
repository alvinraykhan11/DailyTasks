...
public class EditActivity extends AppCompatActivity implements TimePickerDialog.OnTimeSetListener, LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public int insertIntoDatabase() {
        ...
        values.put(ToDoContract.ToDoEntry.COLUMN_EVENT, event_string);
        ...
        if (uri == null) {
            Toast.makeText(this, "Error with inserting DailyTasks",
                    Toast.LENGTH_SHORT).show();
        } else {
            long id_uri = ContentUris.parseId(uri);
            id_todo = (int) id_uri;
            Toast.makeText(this, "DailyTasks Saved",
                    Toast.LENGTH_SHORT).show();
        }
        return ID;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        event.setOnTouchListener(mTouchListener);



        if (mCurrentToDoUri == null) {
            getSupportActionBar().setTitle("Add DailyTasks");

        } else {
            getSupportActionBar().setTitle("Edit DailyTasks");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.save:
                event_string = event.getText().toString().trim();
                message_string = message.getText().toString().trim();
                heading_string = heading.getText().toString().trim();
                if (TextUtils.isEmpty(heading_string) || TextUtils.isEmpty(message_string) || TextUtils.isEmpty(event_string) || TextUtils.isEmpty(time_string) || TextUtils.isEmpty(date_string)) {
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            event_string = cursor.getString(eventColumnIndex);
